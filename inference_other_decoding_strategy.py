...
def main():
    ...
    generation_config_dict = {
        ...
        'beam_search': GenerationConfig(
            do_sample=False,  # beam search is deterministic by default
            num_beams=5
        )
    }
    ...
    for i, data in enumerate(dataloader):
        ...
        pred_base = None
        pred_base_explained = None
        pred_persona = None
        pred_persona_explained = None
        job = None
        ...
        if args.method == 'zero_shot':
            output, base_output_tokens = generation_with_config(
                base_chat, model, tokenizer, generation_config_dict[args.decoding_strategy], device)
            base_chat.append({"role": "assistant", "content": output})
            base_chat.append({"role": "user", "content": args.direct_answer_trigger.strip()})
            pred_full_base, _ = generation(base_chat, model, tokenizer, device)
            pred_base = answer_cleansing(args, pred_full_base)
            pred = clean_pred(pred_base)

        elif args.method == 'role_play':
            ...
            persona_output, persona_output_tokens = generation(
                persona_chat, model, tokenizer, generation_config_dict[args.decoding_strategy], device)
            persona_chat.append({"role": "assistant", "content": persona_output})
            pred_full_persona, _ = generation(persona_chat, model, tokenizer, device)
            pred_persona = answer_cleansing(args, pred_full_persona)
            pred = clean_pred(pred_persona)
        ...
        logfile = f'./logs/{args.decoding_strategy}_{args.method}_{model_name}_{args.dataset}_{args.etc}'
        if args.persona_type == 'fixed':
...
def parse_arguments():
    ...
    parser.add_argument(
        "--decoding_strategy", type=str, choices=['beam_search', 'top_p', 'top_k', 'temperature', 'greedy'], default='greedy'
    )
